package com.orangehaswing.crypto.asymmetric;

import com.orangehaswing.crypto.CryptoException;
import com.orangehaswing.crypto.SecureUtil;
import com.orangehaswing.crypto.asymmetric.SM2Engine.SM2Mode;
...
import static com.orangehaswing.crypto.asymmetric.KeyType.PrivateKey;
import static com.orangehaswing.crypto.asymmetric.KeyType.PublicKey;
...
public class SM2 extends AbstractAsymmetricCrypto<SM2> {
	...
	public byte[] sign(byte[] data, byte[] id) {
		...
		try {
			CipherParameters param = new ParametersWithRandom(getCipherParameters(PrivateKey));
			if (id != null) {
	...
	public boolean verify(byte[] data, byte[] sign, byte[] id) {
		...
		final SM2Signer signer = getSigner();
		try {
			CipherParameters param = getCipherParameters(PublicKey);
			if (id != null) {
				param = new ParametersWithID(param, id);
			}
